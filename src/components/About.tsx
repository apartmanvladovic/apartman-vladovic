...
export function About() {
  ...
  return (
    <section id="o-nama" className="scroll-mt-20 bg-mist-100 py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <dl className="mx-auto mt-14 grid max-w-4xl grid-cols-2 gap-4 lg:grid-cols-4">
          {stats.map(({ icon: Icon, label, value }) => (
            <div
              key={label}
              className="rounded border border-forest-950/10 bg-white p-6 text-center"
            >
              <Icon className="mx-auto h-7 w-7 text-forest-500" aria-hidden />
              <dd className="mt-3 font-display text-2xl text-forest-700">{value}</dd>
              <dt className="mt-1 text-sm text-forest-950/60">{label}</dt>
            </div>
          ))}
        </dl>

        <div className="mx-auto mt-14 max-w-4xl rounded bg-forest-700 p-8 text-white">
          <h3 className="font-display text-xl">Cjenovnik</h3>
